...
router.post('/', async (req, res) => {
  try {
    ...
    const validNeuras = ['CEO', 'CFO', 'CTO', 'IA', 'CISO', 'CSO', 'COO', 'CHRO', 'CMO', 'CDO'];
    ...
    if (webhook_url) {
      try {
        ...
        console.log(`Warning: Webhook test failed for agent ${agentId}:`, error.message);
...
router.post('/:id/execute', async (req, res) => {
  try {
    ...
    executeAgentAsync(executionId, agent, params)
      .catch(error => {
        console.error(`Error executing agent ${id}:`, error);
      });
...
async function executeAgentAsync(executionId, agent, params) {
  ...
  try {
    ...
    console.error(`Agent ${agent.id} execution failed:`, error.message);
  }
}
